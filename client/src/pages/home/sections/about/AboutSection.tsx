...
function AboutSection({ title, cards }: WebsiteDataAboutSection) {
  ...
  useEffect(() => {
    document.addEventListener("scroll", () => {
      ...
      const headingDocHeight =
        (headingBoundingRect.y + headingBoundingRect.bottom) / 2;

      const closestCard = cards.reduce((prev, curr) => {
        const prevDistance =
          headingDocHeight -
          (prev.getBoundingClientRect().y +
            prev.getBoundingClientRect().bottom) /
            2;
        const currDistance =
          headingDocHeight -
          (curr.getBoundingClientRect().y +
            curr.getBoundingClientRect().bottom) /
            2;
